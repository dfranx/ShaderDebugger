...
namespace pp
{
    ...
    void Preprocessor::Process(const std::string& src, const std::string& fname, TokenSequence& os) {
        TokenSequence is;
        IncludeSource(is, &src, &fname);
        Expand(os, is);
        Finalize(os);
    }
    ...
    void Preprocessor::ParseVersion(TokenSequence ls) {
        ...
        size_t end = 0;
        try {
            ver = stoi(tok->str_, &end, 10);
        } catch (const std::out_of_range & oor) {
            Error(tok, "version number out of range");
        }
    ...
    void Preprocessor::IncludeSource(TokenSequence& is, const std::string* src, const std::string* filename)
    {
        TokenSequence ts{ is.tokList_, is.begin_, is.begin_ };
